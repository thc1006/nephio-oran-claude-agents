...
| 代理 | 目的 | 模型 | 能力 |
|-------|---------|-------|--------------|
| [**編排代理**](./orchestration/nephio-oran-orchestrator-agent.md) | 主協調器 | Claude Opus | 多叢集部署、工作流程管理、套件變體編排 |
| [**基礎架構代理**](./infrastructure/nephio-infrastructure-agent.md) | 基礎架構供應 | Claude Sonnet | Kind/Metal3 叢集、Nephio R5 元件、ArgoCD、儲存 |
| [**配置管理代理**](./config-management/configuration-management-agent.md) | 配置部署 | Claude Haiku | Porch 套件、YANG 模型、網路連接、kpt 功能 |
| [**網路功能代理**](./network-functions/oran-network-functions-agent.md) | O-RAN 網路功能 | Claude Haiku | Near-RT RIC、SMO、xApps、rApps、CU/DU/RU 部署 |
| [**監控分析代理**](./monitoring/monitoring-analytics-agent.md) | 可觀察性堆疊 | Claude Sonnet | Prometheus、Grafana、VES 收集器、Jaeger 追蹤 |
| [**數據分析代理**](./analytics/data-analytics-agent.md) | 數據處理 | Claude Sonnet | Kafka 串流、InfluxDB、Flink 處理、ML 流水線 |
| [**安全合規代理**](./security/security-compliance-agent.md) | 安全驗證 | Claude Sonnet | WG11 合規性、FIPS 模式、容器掃描、零信任 |
| [**性能最佳化代理**](./performance/performance-optimization-agent.md) | 性能調整 | Claude Opus | SMO 最佳化、能源效率、AI/ML 加速 |
| [**測試驗證代理**](./testing/testing-validation-agent.md) | 端到端測試 | Claude Haiku | 介面測試、混沌工程、負載測試、Go 覆蓋率 |
| [**相依性診斷代理**](./infrastructure/oran-nephio-dep-doctor-agent.md) | 相依性解決 | Claude Sonnet | Go 模組、Python 套件、系統程式庫、容器修復 |
...
- **[快速開始](./guides/quickstart.md)**：快速設置和基本操作
- **[架構](./architecture/)**：系統設計和元件互動
- **[代理](./agents/)**：個別代理文件和能力
- **[整合](./integration/)**：工作流程指南和最佳實踐
- **[API 參考](./api/)**：完整的 API 文件
- **[範例](./examples/)**：真實世界使用情境
- **[故障排除](./troubleshooting/)**：常見問題和解決方案
...
請參閱我們的[貢獻指南](./CONTRIBUTING.md)了解如何開始的詳細資訊。
...
1. **[快速開始指南](./guides/quickstart.md)** - 幾分鐘內上手並執行
2. **[架構概述](./architecture/)** - 了解系統設計
3. **[代理文件](./agents/)** - 學習個別代理能力
4. **[API 參考](./api/)** - 與我們的 API 整合
5. **[範例](./examples/)** - 查看真實世界使用情境
