...
describe('SupportStatement', () => {
  ...
  describe('Full variant (default)', () => {
    it('renders full variant with all content', () => {
      ...
      expect(
        screen.getByText(/This documentation and the associated Claude agents/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Required Go runtime version/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/O-RAN Alliance L-Release specifications/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Nephio R5 package orchestration/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Configuration as Data package management/)
      ).toBeInTheDocument();
    });
    ...
    it('displays additional info in full variant', () => {
      ...
      expect(
        screen.getByText(/We follow Kubernetes' support policy/)
      ).toBeInTheDocument();
    });
    ...
    it('renders all release badges with correct props', () => {
      ...
      expect(screen.getByTestId('release-badge-oran')).toHaveAttribute(
        'data-variant',
        'outline'
      );
      expect(screen.getByTestId('release-badge-nephio')).toHaveAttribute(
        'data-variant',
        'outline'
      );
      expect(screen.getByTestId('release-badge-kpt')).toHaveAttribute(
        'data-variant',
        'outline'
      );
    });
  ...
  describe('Compact variant', () => {
    it('renders compact variant with reduced content', () => {
      ...
      expect(
        screen.queryByText(
          /This documentation and the associated Claude agents/
        )
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(/While these are the canonical supported versions/)
      ).not.toBeInTheDocument();
    });
  ...
  describe('Badges-only variant', () => {
    it('renders only badges without text content', () => {
      ...
      expect(
        screen.queryByText(/Required Go runtime version/)
      ).not.toBeInTheDocument();
  ...
  describe('Export', () => {
    it('exports SupportStatement as named export', () => {
      const {
        SupportStatement: NamedExport,
      } = require('../../src/components/SupportStatement');
